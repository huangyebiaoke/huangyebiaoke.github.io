...
function loadTipsMessage(result) {
    ...
    if (live2d_settings.showF12OpenMsg) {
        re.toString = function() {
            showMessage(getRandText(result.waifu.console_open_msg), 4000, true);
            return '';
        };
    }
    ...
    if (live2d_settings.showCopyMessage) {
        $(document).on('copy', function() {
            showMessage(getRandText(result.waifu.copy_message), 4000, true);
        });
    }
    
    $('.waifu-tool .fui-photo').click(function(){
        showMessage(getRandText(result.waifu.screenshot_message), 4000, true);
    ...
    window.showWelcomeMessage = function(result) {
        ...
        if (window.location.href == live2d_settings.homePageUrl) {
            ...
            else if (now > 5 && now <= 7) text = getRandText(result.waifu.hour_tips.t5_8);
            else if (now > 7 && now <= 11) text = getRandText(result.waifu.hour_tips.t8_11);
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.textures['id'] == 1 && (modelTexturesId == 1 || modelTexturesId == 0))
                    showMessage(waifu_tips.load_rand_textures_one, 3000, true);
                else showMessage(waifu_tips.load_rand_textures, 3000, true);
    	    ...
    	    case 'lwl12.com':
    	        $.getJSON('https://api.lwl12.com/hitokoto/v1?encode=realjson',function(result){
        	        if (!empty(result.source)) {
                        ...
                        window.setTimeout(function() {showMessage(text+waifu_tips.hitokoto_api_message['lwl12.com'][2], 3000, true);}, 4000);
                    } showMessage(result.text, 4000, true);
                });break;
    	    case 'fghrsh.net':
    	        $.getJSON('https://api.fghrsh.net/hitokoto/rand/?encode=jsc&uid=3335',function(result){
            	    if (!empty(result.source)) {
                        ...
                        window.setTimeout(function() {showMessage(text, 3000, true);}, 4000);
                        showMessage(result.hitokoto, 4000, true);
            	    }
                });break;
            case 'jinrishici.com':
                $.ajax({
                    ...
                    success: function (result, status) {
                        if (!empty(result.data.origin.title)) {
                            ...
                            window.setTimeout(function() {showMessage(text, 3000, true);}, 4000);
                        } showMessage(result.data.content, 4000, true);
                    }
                });break;
    	    default:
